...
- **No accuracy claim without a measurement.** Our measured ANPR read accuracy **misses the challenge’s >90% detection/processing accuracy target, and is reported as missing**. That figure is printed in §5, on the
...
|---|---|---|
| R1 | Overall solution architecture with high-level diagrams and component interactions | §1 |
| R2 | Approach for integrating heterogeneous cameras, NVRs and VMS into a unified platform (IP, analog, multi-vendor, varied protocols) | §2 |
| R3 | Architecture for ingesting, processing and managing live streams from geographically dispersed locations | §3 |
| R4 | Approach for integrating live feeds with watchlist databases and continuously correlating analytics results to generate real-time alerts | §4 |
| R5 | AI-powered analytics approach: ANPR (mandatory), object detection, person/vehicle tracking | §5 |
| R6 | Alert generation and notification workflow: prioritisation, visualisation, user interaction | §6 |
| R7 | Scalability, interoperability, security and performance for statewide deployment | §7 |
| R8 | Technical prerequisites, assumptions, and information required from participating departments | §8 |
| R9 | Assumptions and constraints | §9 |

---

## 1. Overall solution architecture, with high-level diagrams and component interactions
...
*Figure 1 — System context: sources, edge, core, console.*
...
![Edge node](architecture/02-edge-district-node.png)

*Figure 2 — Inside one edge / district node.*

![Data flow](architecture/03-data-flow.png)

*Figure 3 — Data flow: frame to sighting to alert to evidence.*

## 2. Approach for integrating heterogeneous cameras, NVRs and VMS into a unified platform
...
## 3. Architecture for ingesting, processing and managing live streams from geographically dispersed locations
...
| | |
|---|---|
| Every camera streamed to a central VMS — 80,000 × 2 Mbps | **160.00 Gbps** |
| Metadata only, from cameras analysed at the edge | **1.28 Gbps** |
| Total under this architecture | **1.28 Gbps** |
| **Reduction** | **125×** |

**We quote 125×, which is the smaller of two defensible numbers.** It runs on the original
...
2848×. The measured rate makes the architecture look better, not worse, and we lead with
...
![Deployment topology](architecture/04-deployment-topology.png)

*Figure 4 — Deployment topology for statewide operation.*
...
leaves the district. This is the only split that survives the 160.00 Gbps figure above, and it
...
## 4. Approach for integrating live feeds with watchlist databases, and continuously correlating analytics to generate real-time alerts
...
## 5. AI-powered analytics approach: ANPR, object detection, and vehicle tracking
...
| Character accuracy (1 − editDistance/len) | **51.8%** |
| Human-legible plates in the hand-labelled sample | **3 of 120** |
| Against the challenge’s >90% target | **MISSES the challenge’s >90% detection/processing accuracy target, and is reported as missing** |
...
the **24,000 cameras** in the statewide model that would need to be ANPR-viable for it to.
...
## 6. Alert generation and notification workflow: prioritisation, visualisation, user interaction
...
![Audit chain](architecture/06-audit-chain.png)

*Figure 6 — The tamper-evident audit chain and export bundle.*

## 7. Scalability, interoperability, security and performance for statewide deployment
...
|---|---|---|---|
| ANPR cameras | 500 | 24,000 | 30,000 |
| Accelerators required | 63 | 240 | 300 |
| District nodes | 33 | 33 | 33 |
| Events per day | 1,05,84,000 | 1,69,34,40,000 | 2,11,68,00,000 |
| Retained storage | 2–7 TB | 841–3493 TB | 1052–4367 TB |
| Total backhaul | 0.00 Gbps | 0.06 Gbps | 0.07 Gbps |
| Capex | INR 3.07 crore – INR 5.12 crore | INR 11.70 crore – INR 19.50 crore | INR 14.63 crore – INR 24.38 crore |
| Annual opex | INR 49.52 lakh – INR 83.46 lakh | INR 2.23 crore – INR 7.05 crore | INR 2.85 crore – INR 8.93 crore |
| Annual cost per camera | INR 22,189 – INR 37,168 | INR 571 – INR 1,369 | INR 578 – INR 1,380 |
...
![Trust-score pipeline](architecture/05-trust-score-pipeline.png)

*Figure 5 — The trust-score pipeline.*
...
## 8. Technical prerequisites, and the information required from participating departments
...
| # | Constraint | Status |
|---|---|---|
| C1 | **ANPR read accuracy on this estate is 0%** | Accepted and published. A camera-placement finding, not a model finding (§5) |
