...
function costSection(r: SizingResult): string {
  const c = r.cost;
  const lines = c.lines.map((l) => `| ${l.label} | ${formatInrBand(l.inrPerYear)} | ${l.basis} |`);
  return [
    '## 4 · Cost',
    '',
 ...
 *
 * This section reproduces `PROJECT.md` section 9 under **section 9's own constants**. Folding the
 * reader's edits in would make it a reproduction of something else, and the comparison would stop
 * meaning anything.
 */
function section9Section(): string {
...
function provenanceSection(r: SizingResult, overrides: SizingOverrides): string {
  ...
  return [
    ...
    '',
    ...all.filter((c) => c.provenance === 'measured').map((c) => `- **${c.label}** — ${c.note}`),
    '',
  ].join('\n');
}
...
export function renderScenarioMarkdown(options: ScenarioMarkdownOptions): string {
  ...
  if (options.includeSection9 !== false) parts.push(section9Section());
