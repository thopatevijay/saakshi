...
const STATUS_STYLE: Record<AuditEntryStatus, { label: string; className: string; title: string }> = {
  ...
  hash_mismatch: {
    ...
    title: 'This entry does not hash to its stored value — its contents changed after it was written.',
  },
};
...
export function EntryRow({ entry }: { entry: AuditEntry }) {
  ...
  return (
    <tr className="border-t border-slate-800 align-top" data-testid="audit-row" data-status={entry.status}>
      ...
      <td className="px-3 py-2.5 font-mono text-xs whitespace-nowrap text-sky-300">{entry.action}</td>
