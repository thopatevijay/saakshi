...
async function inTamperSandbox(
  body: (tx: Db, seeded: { id: string; hash: string }[]) => Promise<void>,
): Promise<void> {
  try {
    await db.transaction(async (tx) => {
      // `set local` — reverted with the transaction, so the guard is never off outside it.
      await tx.execute('set local role none' as never).catch(() => undefined);
      const seeded: { id: string; hash: string }[] = [];
      for (const n of [1, 2, 3]) {
        const written = await writeAudit(tx as unknown as Db, undefined, {
      ...
      const rows = await tx.select({ id: auditLog.id, hash: auditLog.hash }).from(auditLog).orderBy(asc(auditLog.seq));
      for (const entry of seeded) {
        entry.id = rows.find((r) => r.hash === entry.hash)?.id ?? '';
      }

      await body(tx as unknown as Db, seeded);
      throw new Rollback();
    });
  } catch (error) {
...
/** Disables the append-only trigger for the duration of the transaction only. */
async function withTriggerDisabled(tx: Db, body: () => Promise<void>): Promise<void> {
  await tx.execute('alter table audit_log disable trigger audit_log_no_update' as never);
  try {
    await body();
  } finally {
    await tx.execute('alter table audit_log enable trigger audit_log_no_update' as never);
  }
}
...
describe('a tampered entry is identified, and it is the right one', () => {
  it('the database refuses the UPDATE outright until a guard is disabled', async () => {
    ...
    await inTamperSandbox(async (tx, seeded) => {
      ...
      const refusal = await tx
        .execute(`update audit_log set purpose = 'tampered' where id = '${victim.id}'` as never)
        .then(
          () => null,
          (error: unknown) => error,
        );
  ...
  it('verification names the tampered entry as the first broken link', async () => {
    ...
    await inTamperSandbox(async (tx, seeded) => {
      ...
      expect(before.ok, 'the chain must verify before the tamper, or the test proves nothing').toBe(true);
  ...
  it('it is the FIRST break that is reported, not merely a break', async () => {
    ...
    await inTamperSandbox(async (tx, seeded) => {
      ...
      await withTriggerDisabled(tx, async () => {
        ...
        await tx.execute(`update audit_log set result_count = 99 where id = '${third?.id}'` as never);
        await tx.execute(`update audit_log set result_count = 98 where id = '${second?.id}'` as never);
      });
  ...
  it('removing an entry breaks the LINK, and the entry after it is named', async () => {
    ...
    await inTamperSandbox(async (tx, seeded) => {
      ...
      await tx.execute('alter table audit_log disable trigger audit_log_no_delete' as never);
      try {
        await tx.execute(`delete from audit_log where id = '${second?.id}'` as never);
      } finally {
        await tx.execute('alter table audit_log enable trigger audit_log_no_delete' as never);
      }
